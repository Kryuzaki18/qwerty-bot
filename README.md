...
- **Locations** — capture a set of mouse positions (press `Space` to record the current cursor position, `Escape` to stop), name the set, and save it as a trigger bot. Each saved position can have its own replay delay. Triggering a bot moves the mouse to each position, clicks, and waits before moving to the next.
...
ui/
  src/        React renderer (pages: Dashboard, Locations; commons: Header, Sidebar)
```
