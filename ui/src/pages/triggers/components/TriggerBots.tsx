...
import { MAX_TRIGGER_BOTS } from "../../../constants/trigger.constant";
import {
  ICON_BUTTON,
...
import TriggerBotPositions from "./TriggerBotPositions";
...
function TriggerBots({
  ...
  const { triggerBots, renameBot, reorderBots } = useTriggerBotsStore();
  const {
    isCapturing,
    capturedPositions,
    addingLocationBotId,
  ...
  return (
    <section className="flex min-h-0 flex-col gap-3 bg-neutral-50 dark:bg-neutral-900/30 p-3 rounded-lg">
            ...
            {triggerBots.map((bot, botIndex) => (
              <li
                ...
                className={`rounded-lg border border-neutral-200 p-4 dark:border-neutral-800 bg-white dark:bg-transparent transition-opacity ${
                  ...
                  dragOverBotIndex === botIndex && draggedBotIndex !== botIndex
      ...
      {draggedBotIndex !== null && (
        <DragPreview position={botPointerPosition}>
          <div className="rounded-lg border border-emerald-500 bg-white px-3 py-2 text-sm font-medium text-neutral-900 shadow-xl dark:bg-neutral-900 dark:text-neutral-100">
            {triggerBots[draggedBotIndex]?.name}
          </div>
        </DragPreview>
      )}
    </section>
